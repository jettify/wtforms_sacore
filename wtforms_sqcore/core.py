...
class ModelConverterBase(object):
    ...
    def convert(self, table, column, field_args):
        ...
        if not hasattr(column, 'direction'):
        ...
        return converter(table=table, column=column, field_args=kwargs)
...
class ModelConverter(ModelConverterBase):
    ...
    def conv_String(self, field_args, **extra):
        ...
        return f.StringField(**field_args)
...
def model_fields(table, only=None, exclude=None, field_args=None,
                 converter=None, exclude_pk=False, exclude_fk=False):
    """
    ...
    for column in table.columns:
        if exclude_fk and column.foreign_keys:
            import ipdb; ipdb.set_trace()

            continue
        elif exclude_pk and column.primary_key:
                continue
    ...
    if only:
        columns = (x for x in columns if x[0] in only)
    elif exclude:
        columns = (x for x in columns if x[0] not in exclude)
    field_dict = {}
    for name, column in columns:
        field = converter.convert(table, column, field_args.get(name))
...
def model_form(table, base_class=Form, only=None,
    ...
    """
    type_name = type_name or str(table.name + 'Form')
    field_dict = model_fields(
        table, only, exclude, field_args, converter,
